...
export const PartnerSelector: React.FC<PartnerSelectorProps> = ({
  ...
  const validParceiros = parceiros?.filter(
    (p: ParceiroItem) => p.parceiroId && p.Parceiro?.nome
  ) || [];
  ...
  if (!hasPartners || !parceiros || parceiros.length === 0 || validParceiros.length === 0) {
    return (
      <div className="text-sm text-muted-foreground">
        {t('common.noPartnersAvailable', { defaultValue: 'Nenhum parceiro disponível' })}
      </div>
    );
  }
      ...
      >
        ...
        <SelectContent>
          ...
          {validParceiros.map((parceiro: ParceiroItem) => (
            <SelectItem key={parceiro.parceiroId} value={parceiro.parceiroId!.toString()}>
