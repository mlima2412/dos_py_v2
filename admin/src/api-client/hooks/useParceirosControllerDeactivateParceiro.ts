 ...
 */
export async function parceirosControllerDeactivateParceiro(
  publicId: ParceirosControllerDeactivateParceiroPathParams['publicId'],
  config: Partial<RequestConfig> & { client?: typeof fetch } = {},
) {
  const { client: request = fetch, ...requestConfig } = config

  const res = await request<ParceirosControllerDeactivateParceiroMutationResponse, ResponseErrorConfig<ParceirosControllerDeactivateParceiro404>, unknown>({
    method: 'PATCH',
    url: `/parceiros/${publicId}/desativar`,
    ...requestConfig,
  })
  return res.data
}
 ...
 * {@link /parceiros/:publicId/desativar}
 */
export function useParceirosControllerDeactivateParceiro<TContext>(
  options: {
    mutation?: UseMutationOptions<
      ParceirosControllerDeactivateParceiroMutationResponse,
      ResponseErrorConfig<ParceirosControllerDeactivateParceiro404>,
      { publicId: ParceirosControllerDeactivateParceiroPathParams['publicId'] },
      TContext
    > & { client?: QueryClient }
