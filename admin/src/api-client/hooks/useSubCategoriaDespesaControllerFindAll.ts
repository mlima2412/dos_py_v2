/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import fetch from '@/lib/fetch-client'
import type {
  SubCategoriaDespesaControllerFindAllQueryResponse,
  SubCategoriaDespesaControllerFindAll401,
} from '../types/SubCategoriaDespesaControllerFindAll.ts'
import type { RequestConfig, ResponseErrorConfig } from '@/lib/fetch-client'
import type { QueryKey, QueryClient, QueryObserverOptions, UseQueryResult } from '@tanstack/react-query'
import { queryOptions, useQuery } from '@tanstack/react-query'

export const subCategoriaDespesaControllerFindAllQueryKey = () => [{ url: '/subcategoria-despesa' }] as const

export type SubCategoriaDespesaControllerFindAllQueryKey = ReturnType<typeof subCategoriaDespesaControllerFindAllQueryKey>

/**
 * @summary Listar todas as subcategorias de despesas ativas
 * {@link /subcategoria-despesa}
 */
export async function subCategoriaDespesaControllerFindAll(config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config

  const res = await request<SubCategoriaDespesaControllerFindAllQueryResponse, ResponseErrorConfig<SubCategoriaDespesaControllerFindAll401>, unknown>({
    method: 'GET',
    url: `/subcategoria-despesa`,
    ...requestConfig,
  })
  return res.data
}

export function subCategoriaDespesaControllerFindAllQueryOptions(config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  const queryKey = subCategoriaDespesaControllerFindAllQueryKey()
  return queryOptions<
    SubCategoriaDespesaControllerFindAllQueryResponse,
    ResponseErrorConfig<SubCategoriaDespesaControllerFindAll401>,
    SubCategoriaDespesaControllerFindAllQueryResponse,
    typeof queryKey
  >({
    queryKey,
    queryFn: async ({ signal }) => {
      config.signal = signal
      return subCategoriaDespesaControllerFindAll(config)
    },
  })
}

/**
 * @summary Listar todas as subcategorias de despesas ativas
 * {@link /subcategoria-despesa}
 */
export function useSubCategoriaDespesaControllerFindAll<
  TData = SubCategoriaDespesaControllerFindAllQueryResponse,
  TQueryData = SubCategoriaDespesaControllerFindAllQueryResponse,
  TQueryKey extends QueryKey = SubCategoriaDespesaControllerFindAllQueryKey,
>(
  options: {
    query?: Partial<
      QueryObserverOptions<
        SubCategoriaDespesaControllerFindAllQueryResponse,
        ResponseErrorConfig<SubCategoriaDespesaControllerFindAll401>,
        TData,
        TQueryData,
        TQueryKey
      >
    > & { client?: QueryClient }
    client?: Partial<RequestConfig> & { client?: typeof fetch }
  } = {},
) {
  const { query: { client: queryClient, ...queryOptions } = {}, client: config = {} } = options ?? {}
  const queryKey = queryOptions?.queryKey ?? subCategoriaDespesaControllerFindAllQueryKey()

  const query = useQuery(
    {
      ...subCategoriaDespesaControllerFindAllQueryOptions(config),
      queryKey,
      ...queryOptions,
    } as unknown as QueryObserverOptions,
    queryClient,
  ) as UseQueryResult<TData, ResponseErrorConfig<SubCategoriaDespesaControllerFindAll401>> & { queryKey: TQueryKey }

  query.queryKey = queryKey as TQueryKey

  return query
}
