...
export async function authControllerRefresh(
  ...
  config: Partial<RequestConfig<AuthControllerRefreshMutationRequest>> & { client?: typeof fetch } = {},
) {
  const { client: request = fetch, ...requestConfig } = config

  const requestData = data
  const res = await request<AuthControllerRefreshMutationResponse, ResponseErrorConfig<AuthControllerRefresh401>, AuthControllerRefreshMutationRequest>({
    method: 'POST',
    url: `/auth/refresh`,
    data: requestData,
    ...requestConfig,
