 ...
 * {@link /clientes}
 */
export async function clientesControllerCreate(
  data: ClientesControllerCreateMutationRequest,
  config: Partial<RequestConfig<ClientesControllerCreateMutationRequest>> & { client?: typeof fetch } = {},
) {
  ...
  const res = await request<
    ClientesControllerCreateMutationResponse,
    ResponseErrorConfig<ClientesControllerCreate400 | ClientesControllerCreate409>,
    ClientesControllerCreateMutationRequest
  >({ method: 'POST', url: `/clientes`, data: requestData, ...requestConfig })
  return res.data
}
 ...
 * {@link /clientes}
 */
export function useClientesControllerCreate<TContext>(
  options: {
