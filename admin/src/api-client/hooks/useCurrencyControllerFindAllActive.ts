...
export async function currencyControllerFindAllActive(config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<CurrencyControllerFindAllActiveQueryResponse, ResponseErrorConfig<CurrencyControllerFindAllActive401>, unknown>({
    method: 'GET',
    url: `/currency/ativos`,
    ...requestConfig,
  })
...
export function currencyControllerFindAllActiveQueryOptions(config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  ...
  return queryOptions<
    ...
    ResponseErrorConfig<CurrencyControllerFindAllActive401>,
    CurrencyControllerFindAllActiveQueryResponse,
    typeof queryKey
  >({
    queryKey,
 ...
 * {@link /currency/ativos}
 */
export function useCurrencyControllerFindAllActive<
  TData = CurrencyControllerFindAllActiveQueryResponse,
  TQueryData = CurrencyControllerFindAllActiveQueryResponse,
  TQueryKey extends QueryKey = CurrencyControllerFindAllActiveQueryKey,
>(
  options: {
    query?: Partial<
      QueryObserverOptions<CurrencyControllerFindAllActiveQueryResponse, ResponseErrorConfig<CurrencyControllerFindAllActive401>, TData, TQueryData, TQueryKey>
    > & { client?: QueryClient }
  ...
  const query = useQuery(
    ...
    queryClient,
  ) as UseQueryResult<TData, ResponseErrorConfig<CurrencyControllerFindAllActive401>> & { queryKey: TQueryKey }
