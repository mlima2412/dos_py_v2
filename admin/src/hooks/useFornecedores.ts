...
export function useFornecedores(params: UseFornecedoresParams = {}) {
  ...
  return useInfiniteQuery({
    queryKey: ['fornecedores', { search, ativo, limit }],
    queryFn: async ({ pageParam = 1 }) => {
      ...
      if (search) {
        filteredFornecedores = allFornecedores.filter(fornecedor =>
      ...
      // Filtrar por ativo
      if (ativo !== undefined) {
        filteredFornecedores = filteredFornecedores.filter(fornecedor => fornecedor.ativo === ativo);
      }
