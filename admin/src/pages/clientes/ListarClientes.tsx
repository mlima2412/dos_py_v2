...
export const ListarClientes: React.FC = () => {
	...
	// Buscar clientes com scroll infinito
	const {
		data: clientesData,
		fetchNextPage,
		hasNextPage,
	...
	const data = useMemo(() => {
		return (
			clientesData?.pages.flatMap(page => page.data || []).filter(Boolean) || []
		);
	}, [clientesData]);

	// Total de clientes
	const total = clientesData?.pages[0]?.total || 0;
				...
				<div className="flex justify-between items-center">
					<Breadcrumb>
						<BreadcrumbList>
							...
							<BreadcrumbSeparator />
							<BreadcrumbItem>
								<BreadcrumbPage>{t("clients.title")}</BreadcrumbPage>
							</BreadcrumbItem>
						</BreadcrumbList>
					</Breadcrumb>
