...
export function FormularioUsuario() {
  ...
  const [newAssociation, setNewAssociation] = useState<Omit<ParceiroPerfilAssociation, 'id'>>({ parceiroId: '', perfilId: '' });
  ...
  const { data: userData, isLoading: isLoadingUser } = useUsuariosControllerFindOne(
    publicId || '',
    {
      query: {
  ...
  const addAssociationMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      const parceiro = parceiros?.find(p => p.id.toString() === newAssociation.parceiroId);
      const perfil = profiles?.find(p => p.id.toString() === newAssociation.perfilId);
      
      setAssociations(prev => [...prev, {
        id: response.id,
        parceiroId: newAssociation.parceiroId,
        perfilId: newAssociation.perfilId,
        parceiro: parceiro ? { id: parceiro.id, nome: parceiro.nome } : undefined,
  ...
  const addAssociation = () => {
    ...
    const exists = associations.some(
      a => a.parceiroId === newAssociation.parceiroId && a.perfilId === newAssociation.perfilId
    );
  ...
  const updateUserMutation = useMutation({
    mutationFn: async (data: UpdateUserFormData) => {
      ...
      // Atualizar apenas dados básicos do usuário
       const response = await usuariosControllerUpdate(
         publicId,
         {
           publicId: publicId,
           nome: data.nome,
              ...
              {!isEditing ? (
                ...
                <div className="space-y-4">
                  <div className="border-t pt-6">
                    <h3 className="text-lg font-medium">{t('users.initialPartnerProfile', { defaultValue: 'Parceiro e Perfil Inicial' })}</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          <SelectTrigger>
                            <SelectValue placeholder={t('users.selectPartner', { defaultValue: 'Selecione um parceiro' })} />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      {/* Seleção de Perfil */}
                      <div className="space-y-2">
                        <Label>{t('profiles.title', { defaultValue: 'Perfil' })} *</Label>
                        <Select
                          value={initialProfile}
                          onValueChange={(value) => setInitialProfile(value)}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder={t('users.selectProfile', { defaultValue: 'Selecione um perfil' })} />
                          </SelectTrigger>
                          <SelectContent>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {t('users.initialPartnerInfo', { defaultValue: 'Selecione um parceiro e perfil inicial. Você poderá adicionar mais associações após criar o usuário.' })}
                    </p>
                  </div>
                </div>
              ) : (
                ...
                <div className="space-y-4">
                  <div className="border-t pt-6">
                    <h3 className="text-lg font-medium">{t('partners.title', { defaultValue: 'Parceiros' })}</h3>
                    
                    {/* Lista de associações existentes */}
                    {associations.length > 0 && (
                      <div className="space-y-3 mb-6">
                        <Label>{t('users.existingAssociations', { defaultValue: 'Associações existentes' })}:</Label>
                        <div className="space-y-2">
                          {associations.map((association, index) => (
                            <div key={index} className="flex items-center justify-between p-3 border rounded-lg bg-muted/50">
                              <div className="flex items-center space-x-3">
                                <Badge variant="outline">
                                  {association.parceiro?.nome || parceiros?.find(p => p.id.toString() === association.parceiroId)?.nome}
                                </Badge>
                                <span className="text-muted-foreground">→</span>
                                <Badge variant="secondary">
                                  {association.perfil?.nome || profiles?.find(p => p.id.toString() === association.perfilId)?.nome}
                                </Badge>
                              </div>
                              <Button
                                type="button"
                    ...
                    <div className="space-y-4 p-4 border rounded-lg bg-background">
                      <div className="flex items-center justify-between">
                        <Label>{t('partners.title')} {t('common.available', { defaultValue: 'disponíveis' })}:</Label>
                        <Button
                          type="button"
                          onClick={addAssociation}
                          size="sm"
                          disabled={!newAssociation.parceiroId || !newAssociation.perfilId}
                        >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <Select
                            ...
                            onValueChange={(value) => {
                              setNewAssociation(prev => ({ ...prev, parceiroId: value }));
                              setAssociationErrors('');
                            }}
                          >
                            <SelectTrigger>
                            ...
                            <SelectContent>
                              {parceiros?.filter(parceiro => 
                                !associations.some(assoc => assoc.parceiroId === parceiro.id.toString())
                              ).map((parceiro) => (
                                <SelectItem key={parceiro.id} value={parceiro.id.toString()}>
                                  {parceiro.nome}
                                </SelectItem>
                        ...
                        <div className="space-y-2">
                          ...
                          <Select
                            ...
                            onValueChange={(value) => {
                              setNewAssociation(prev => ({ ...prev, perfilId: value }));
                              setAssociationErrors('');
                            }}
                          >
                            <SelectTrigger>
                      ...
                      {/* Erro de validação */}
                      {associationErrors && (
                        <p className="text-sm text-destructive">{associationErrors}</p>
                      )}

                      {/* Mensagem informativa */}
                      {associations.length === 0 && (
                        <p className="text-sm text-muted-foreground">
                          {t('users.associationRequired', { defaultValue: 'É necessário adicionar pelo menos uma associação parceiro-perfil.' })}
                        </p>
                      )}
                    </div>
                  </div>
