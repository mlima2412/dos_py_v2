...
export const useResponsiveColumns = (t: (key: string) => string, isMobile: boolean = false) => {
  ...
  const updateContasPagarParcelasMutation = useContasPagarParcelasControllerUpdate();
  ...
  const handleMarkAsPaid = async (despesaPublicId: string) => {
    try {
      ...
      const despesa = await despesasControllerFindOne(despesaPublicId, { 'x-parceiro-id': Number(selectedPartnerId) });
      ...
      const { contasPagarControllerFindByDespesa } = await import('@/api-client/hooks');
      const contasPagarResponse = await contasPagarControllerFindByDespesa(despesa.id);
      ...
      if (contasPagarResponse && contasPagarResponse.length > 0) {
        ...
        const { contasPagarParcelasControllerFindByContasPagar } = await import('@/api-client/hooks');
        const parcelas = await contasPagarParcelasControllerFindByContasPagar(contasPagar.id);
        ...
        await queryClient.invalidateQueries({ queryKey: ['despesas'] });
        await queryClient.invalidateQueries({ queryKey: ['contas-pagar'] });
        await queryClient.invalidateQueries({ queryKey: ['contas-pagar-parcelas'] });
  ...
  return useMemo(() => {
    const allColumns = createColumns(t, handleDelete, isAdmin, handleMarkAsPaid);
